...
export function ChatView({ messages, error, scrollRef, isStreaming, pendingWrite, onWriteAccept, onWriteReject, writeDiffRef, voiceRejecting, onRewind, collapseTools }: Props) {
  return (
    <div
      ref={scrollRef}
      style={{
    ...
    >
      {messages.length === 0 ? (
        ...
        <>
          {messages.map((msg) => (
            <MessageBubble key={msg.id} message={msg} onRewind={onRewind} collapseTools={collapseTools} />
          ))}
        </>
      )}
